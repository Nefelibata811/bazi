...
默认已配置「八字」风格图标；要换成自己的图见 [APP_ICON.md](APP_ICON.md)。

```powershell
# 1. 把你的 1024x1024 PNG 放到 assets/icon/app_icon.png
# 2. 生成各尺寸并重新打包
.\scripts\update_app_icon.ps1
.\scripts\build_android.ps1
```

安装新 APK 前建议先卸载旧版，否则桌面可能仍显示缓存的旧图标。

## Android Studio 打包 APK

见 [ANDROID_STUDIO_APK.md](ANDROID_STUDIO_APK.md)。
